...
function ProductList(args) {
  ...
  const handlesearchbylanguage = () => {
    if (language !== "" && language !== undefined) {
      axios
        .post(
          `http://13.233.99.196:9000/user/advancefilter?sub_category=${Params.id}&type=${type}&format=${format}&language=${language}&relYear=${contentyear}`
        )
        .then((res) => {
          console.log(res.data.data);
      ...
      axios
        .post(
          `http://13.233.99.196:9000/user/promotion_filter?sub_category=${Params.id}&type=${type}&format=${format}&language=${language}&relYear=${contentyear}`
        )
        .then((res) => {
          console.log(res.data.data);
  ...
  const handleSelection = (_id) => {
    ...
    if (selectedId === _id) {
      ...
      axios
        .get(`http://13.233.99.196:9000/user/average_rating/${selectedId}`)
        .then((res) => {
          console.log(res.data);
          setAverageRating(res.data);
        })
